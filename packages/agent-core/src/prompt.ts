...
const ERDOU_ABOUT = [
  ...
  "- The preview serves your app through a Service Worker that SYNTHESIZES each response from the runtime — so a guest server's `Set-Cookie` is NOT stored by the browser and cookie-based sessions do NOT persist (every request arrives cookie-less, so the server keeps re-creating state). Keep session/game state in an EXPLICIT token the client sends on each request (a JSON body field, a header, or a query param) or in the page's localStorage — never rely on cookies for state in the preview.",
