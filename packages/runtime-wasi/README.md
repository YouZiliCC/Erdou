...
Implements a practical subset of `wasi_snapshot_preview1`: args, environ, stdio (`fd_read`/`fd_write`), files (`path_open`, `fd_seek`, `fd_close` flushing back to the VFS), `path_filestat_get`, `path_create_directory`/`unlink`, `clock_time_get`, `random_get`, and directory iteration (`fd_readdir` — synthesizing `.`/`..`, with cookie-based resumption and spec-correct truncation when the buffer is too small). Unimplemented calls return `ENOSYS` rather than failing instantiation.
